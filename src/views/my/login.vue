...
export default {
  ...
  methods: {
    ...
    async login () {
      ...
      if (res.data.code === 0) {
        // 登录成功之后获取用户信息
        let userInfo = await F700104({ channel_type: '2' });
        if (userInfo.data.code === 0) {
          const {web_account} = userInfo.data.data;
          // 保存互联网账号到缓存里面，在我的页面请求我的组合的时候用作F700116的入参；
          sessionStorage.setItem('web_account',web_account);
          // 登录成功之后跳转到我的页面
          this.$router.push({
            name:'My'
          })
